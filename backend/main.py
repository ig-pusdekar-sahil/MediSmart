...
async def chat(request: ChatRequest):
    try:
        ...
        # Initialize chat with history and send message with fallback
        try:
...
async def generate_pdf(data: dict, background_tasks: BackgroundTasks):
    try:
        # Create PDF
        buffer = io.BytesIO()
        ...
        manufacturer = data.get('medicine_info', {}).get('manufacturer', '')
        batch_number = data.get('medicine_info', {}).get('batch_number', '')
        expiry_date = data.get('medicine_info', {}).get('expiry_date', '')
        if manufacturer or batch_number or expiry_date:
            elements.append(Paragraph("<b>Manufacturer & Batch Details:</b>", styles['Heading3']))
            if manufacturer:
                elements.append(Paragraph(f"• Manufacturer: {manufacturer}", styles['Normal']))
            if batch_number:
                elements.append(Paragraph(f"• Batch No.: {batch_number}", styles['Normal']))
            if expiry_date:
                elements.append(Paragraph(f"• Expiry Date: {expiry_date}", styles['Normal']))
            elements.append(Spacer(1, 0.15*inch))
